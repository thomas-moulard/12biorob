...
\begin{figure}[ht!]
  \begin{center}
    \begin{tikzpicture}[x=.40\textwidth,y=2.5cm]
      ...
      \path node (plus) at (-0.4,0) [shape=rectangle,draw,color=white]
      ...
      \path node (errorcomp) at (-0.15,0.6)
            ...
            {$\begin{array}{c} 
                k_x \Delta x\\
                k_y \Delta y\\
                k_{\theta} \Delta \theta\end{array}$};

      \draw[-] (-0.4,0.6) -- (errorcomp.west);
      \draw[->] (-0.4,0.6) -- (-0.4,0.07);

      \draw[-] (errorcomp.east) -- (0.1,0.6);
      \draw[-] (0.1,0.6) -- (0.1,0.);
      \draw[->] (robot.east) -- (0.2,0.);

      \draw[->] (-0.4,0.) -- (robot.west);

      \draw[->] (-0.6,0) -- (-0.424,0.) node [above left]
           {$\mathbf{\dot{\mathbf{x}}}$};

      \draw[->] (-0.6,0.7) -- (errorcomp.160) node [above left]
           {$\mathbf{x}^{\text{ref}}$};
    \end{tikzpicture}
  \end{center}
...
makes the naive solution unsatisfying. To solve these issues, a better
